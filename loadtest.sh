...
loadtest -c $CONCURRENCY --cores $CORES -t $TEST_TIME --timeout $TIMEOUT "http://$TEST_IP/" --quiet >> $LOG_FILE

echo "Testing http://$TEST_IP/api single client"
loadtest -c $CONCURRENCY --cores $CORES -t $TEST_TIME --timeout $TIMEOUT "http://$TEST_IP/api?foo=bar" --quiet >> $LOG_FILE

echo "Testing http://$TEST_IP/ws single client"
loadtest -c $CONCURRENCY --cores $CORES -t $TEST_TIME --timeout $TIMEOUT "ws://$TEST_IP/ws" --quiet 2> /dev/null >> $LOG_FILE

echo "Testing http://$TEST_IP/alien.png single client"
loadtest -c $CONCURRENCY --cores $CORES -t $TEST_TIME --timeout $TIMEOUT "http://$TEST_IP/alien.png" --quiet >> $LOG_FILE
