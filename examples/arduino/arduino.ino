...
AuthenticationMiddleware basicAuthMiddleware;
AuthenticationMiddleware digestAuthMiddleware;
...
// look up our keys?
    ...
    // how to do basic auth (v2.x middleware approach)
    basicAuthMiddleware.setUsername(app_user).setPassword(app_pass).setRealm(app_name).setAuthMethod(BASIC_AUTH).setAuthFailureMessage("You must log in.");
    ...
    // how to do digest auth (v2.x middleware approach)
    digestAuthMiddleware.setUsername(app_user).setPassword(app_pass).setRealm(app_name).setAuthMethod(DIGEST_AUTH).setAuthFailureMessage("You must log in.");
    server.on("/auth-digest", HTTP_GET, [](PsychicRequest* request, PsychicResponse* response) {
            return response->send("Auth Digest Success!");
          })
      ->addMiddleware(&digestAuthMiddleware);
