...
mg_str mg_mk_str_from_offsets(mg_str &dest, mg_str &src, mg_str &value) {
  ...
  DBUGF("value = %d %p", value.len, value.p);
  DBUGF("src = %d %p", src.len, src.p);
  DBUGF("dest = %d %p", dest.len, dest.p);
  DBUGF("offset = %d %p", value.p - src.p, (dest.p + (value.p - src.p)));

  s.p = value.p ? (dest.p + (value.p - src.p)) : NULL;
  s.len = value.len;

  DBUGF("s = %d %p", s.len, s.p);
