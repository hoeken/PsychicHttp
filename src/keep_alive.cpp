...
{
    client_fd_action_t client_fd_action = {.type = CLIENT_FD_ADD, .fd = fd};
...
{
    client_fd_action_t client_fd_action = {.type = CLIENT_FD_REMOVE, .fd = fd};
    if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
        return ESP_OK;
    }
    return ESP_FAIL;
...
{
    client_fd_action_t client_fd_action = {.type = CLIENT_UPDATE, .fd = fd, .last_seen = _tick_get_ms()};
    if (xQueueSendToBack(h->q, &client_fd_action, 0) == pdTRUE) {
        return ESP_OK;
    }
    return ESP_FAIL;
